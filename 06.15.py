...
yamls = '[{"identifier":"myadp_wallet_pass","entity_ref":"api:account.default.ADP_IDP_POV/myadp_wallet_pass", ... }]'

# Step 1: Load the list of items (dictionaries) from the JSON string
items = json.loads(yamls)

# Step 2: Parse the 'yaml' field of each item using PyYAML
parsed_yaml_objects = []

for item in items:
    raw_yaml = item.get("yaml", "")
    if raw_yaml:
        # Unescape double-backslashes and parse YAML
        unescaped_yaml = raw_yaml.encode().decode('unicode_escape')
        try:
            parsed = yaml.safe_load(unescaped_yaml)
            parsed_yaml_objects.append(parsed)
        except yaml.YAMLError as e:
            print(f"Failed to parse YAML for identifier: {item.get('identifier')}")
            print(e)

# Step 3: Use or print the results
